...
#models are meant simplify the problem and predict

mod1 <- glm(data = mpg, formula = cty ~poly(displ,2))
class(mod1)

#for this the displ value means that if we go up one vale we lose -2.6
mod1$coefficients
...
#adds column cyl to model above, uses cyl and displ to predict city
mod2 <- glm(data = mpg, formula = cty ~displ + cyl) 

#this makes cyl an interaction variable. What is the influence on 
#cylinder on city
...
#don't want your model to be overfitted, this can happen by adding things that 
#are not relavent to the data but make it more accurate for that specfic data

#need to go over with Zahm again to understand table
compare_models(mod1, mod2, mod3)
  
#R^2 is the improvement over the mean
#AIC smaller is better and takes into account the complexity of model
compare_performance(mod1, mod2, mod3)

#this shows how each model compares visually, model 3 is best overall
compare_performance(mod1, mod2, mod3) %>% plot()

#predict new values or current data set with model

#this prects city based on displ
predict(mod1, mpg)
mod1$formula

#takes dataframe of displ and ppredicts mpg based on cylinders
predict(mod1, data.frame(displ = 1:100))

#gives nonsense answwers, need more variables to makeit realistic
#the plot shows that the model is limited in predicting by the variables we use
#can't predict outside of  the range of the data
...
#black is predicted 
#shows that lower cylinders are not well predicted 
mpg %>% 
...
#AIC andBIC model complexity
compare_performance(mod1, mod2, mod3, mod4) %>% plot()

summary(mod4)

#disclaimer, know how to usethis otherwise it could screw you
#the step function could lead to over fitting if it adds
#variables that may be accidentally correlated
library(MASS)

step <- stepAIC(mod4)
step$formula

mod5 <- glm(data = mpg, formula = cty ~ displ + year + cyl * trans * drv + year) 
mod5$formula
...
mod_best <- glm(data = mpg, formula = formula(step5))

compare_performance(mod1, mod2, mod3, mod4, mod5, mod_best) %>% plot()

formula(step)

#this is only one way of modeling, based ib central limit theorem. 
#we can change what it means to be a line, the backround distributions

#visualize, transform 
#addituve versus interactive points 
